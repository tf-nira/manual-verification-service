...
public class CryptoCoreUtil {
	...
	public static void setPropertyValues() {
		...
		try {
			prop.load(in.tf.nira.manual.verification.util.CryptoCoreUtil.class.getClassLoader()
	...
	public String decrypt(String responseData) throws Exception {
		KeyStore.PrivateKeyEntry privateKey = getPrivateKeyEntryFromP12();
		byte[] responseBytes = Base64.decodeBase64(responseData);
		byte[] deryptedData = decryptData(responseBytes, privateKey);
		return new String(deryptedData);
	}
	...
	private byte[] decryptData(byte[] responseData, KeyStore.PrivateKeyEntry privateKey) throws Exception {
		...
		try {
			...
			byte[] encryptedData = Arrays.copyOfRange(responseData, keyDemiliterIndex + keySplitterLength,
			...
			if (Arrays.equals(headerBytes, VERSION_RSA_2048)) {
				...
				byte[] encData = Arrays.copyOfRange(encryptedData, 32, encryptedData.length);
				...
				return decryptData(secretKey, encData, nonce, aad);
			}
			...
			return decryptData(symmetricKey, encryptedData);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private byte[] decryptData(SecretKey key, byte[] data) throws Exception {
		try {
	...
	private byte[] decryptData(SecretKey key, byte[] data, byte[] nonce, byte[] aad) throws Exception {
		try {
			Cipher cipher = Cipher.getInstance("AES/GCM/PKCS5Padding");
